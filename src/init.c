    ...
    kintbl_struct kintbl[], rttbl_struct *rttbl, subcatch_struct subcatch[])
{
    ...
    for (ksub = 0; ksub < nsub; ksub++)
    {
        ...
        {
            // Apply calibration
            subcatch[ksub].chms[SURFACE].ssa[kspc] *= (chemtbl[kspc].itype == MINERAL) ? calib->ssa : 1.0;   // 2021-05-07
    ...
    const ctrl_struct *ctrl, chmstate_struct *chms)
{
    ...
    for (kspc = 0; kspc < rttbl->num_stc; kspc++)
    {
        ...
        else if (chemtbl[kspc].itype == MINERAL)
        {
            ...
            //concentration (mole of mineral/L of pore space) = Absolute mineral volume fraction (cm3 of mineral/cm3 of media)*1000 (cm3/l)/
          //molar volume (cm3 of mineral/mole of mineral)/porosity (cm3 of pore space/cm3 of media)
